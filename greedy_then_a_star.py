...
def greedy_forwarding_then_a_star(id1, id2, graph, min_velocity, ratio_travelled=False): # id1 is start node id2 is go to node
  inf = np.inf
  ...
  while (current_node != id2):
    ...
    if node_with_min_distance in visited or min_distance == inf: # if empty next sequence then already stop here else a star won't even find a path
      ...
      """
      sec_travelled += a_star.A_star(current_node, id2, graph, min_velocity)
      if ratio_travelled:
